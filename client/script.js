...
// sayHelloColor()
sayHelloBtn.addEventListener('mouseover', sayHelloColor)
...
// CODE HERE
function sayHelloColorBack(event){
...
sayHelloBtn.addEventListener('mouseout', sayHelloColorBack)
...
const repeatMyParam = () => {
    ...
    const repeatParam = document.getElementById('repeat-text');
...
function queryTest(event) {
    axios.get('http://localhost:3000/query-test/?name=Walter')
        .then(res => {
            console.log(res.data)
        })
}
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        Answer= It says that I sent an empty query.

    2: Send more than 1 query on the URL -- what happened? 
        Answer= Doesnt work? I cant seem to get it to work
*/
